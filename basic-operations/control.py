...
def setAltHoldMode():
    ...
    try:
        flightModeService = rospy.ServiceProxy('/mavros/set_mode', mavros_msgs.srv.SetMode)
        #http://wiki.ros.org/mavros/CustomModes for custom modes
        isModeChanged = flightModeService(custom_mode='ALT_HOLD') #return true or false
        rospy.loginfo('Mode Alt Holding...')
    except rospy.ServiceException, e:
        print "service set_mode call failed: %s. ALT_HOLD Mode could not be set. Check that GPS is enabled"%e
...
if __name__ == '__main__':
    rospy.init_node('dronemode_node', anonymous=True)
