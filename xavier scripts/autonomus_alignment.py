...
def alignment_test(h):
    global target_pool_color
    global okColor
    global msg_for_img

    list1 = []
    list1.append(Waypoint(0, 0, h, 0))
    list1.append(Waypoint(0, 5, h, 0))
    counter = 0
    msg_for_img = "0"
    alg_red = False
    alg_blue = False
    while not rospy.is_shutdown():
        ...
        rospy.sleep(.2)
        if okColor == "Red" and not alg_red and msg_for_img == "1":
            rospy.sleep(1.0)
            ...
            if alignment_red():
                ...
                alg_red = True     
        if okColor == "Blue" and not alg_blue and msg_for_img == "0":
            rospy.sleep(1.0)
            target_pool_color = "Blue"
            stop()
            if alignment_blue():
